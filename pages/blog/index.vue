  ...
  <div class="py-4">
    ...
    <div class="flex lg:flex-row-reverse flex-col">
        <div v-if="sidebar && showsidebar" class="lg:w-1/3 px-7 lg:pt-5 flex flex-col gap-y-2 w-full z-50 bg-white h-fit">
            ...
            <div v-if="showcategories" class="flex flex-col gap-y-3">
                <h3 class="font-bold text-lg">{{ $settings.blog.categories.title }}</h3>
        ...
        <div class="lg:border-l px-6 lg:w-full">
            <div :class="!sidebar ? 'justify-between' : 'justify-end'" class="flex bg-slate-100 rounded-md p-3 px-5 my-5 mx-3">
...
export default {
    data(){
        return{
            ...
            showsidebar: this.$settings.blog.show.sidebar,
            showcategories: this.$settings.blog.show.categories,
            ...
            sorts: [
                { field: { 'name': 1 }, name: this.$settings.blog.sort.name_asc },
                { field: { 'name': -1 }, name: this.$settings.blog.sort.name_desc },
                { field: { createdAt: -1 }, name: this.$settings.blog.sort.newest },
                { field: { createdAt: 1 }, name: this.$settings.blog.sort.oldest }
            ],
        }
    },
    mounted() {
    ...
    methods:{
        ...
        setParams(e, key, value){
            ...
            switch(key){
                case 'categories.slug-in': this.param = this.params['collections.slug-in'].push(value);break;
            }
        },
        async getItems(){
            this.loading = true;
