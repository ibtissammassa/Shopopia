<template>
  <div class="pt-16 pb-10 px-6 md:px-16 lg:px-10 gap-y-9 flex flex-col justify-center">
    <h2 class="text-2xl md:text-3xl font-bold">{{ title }}</h2>
    <div v-if="items.length >0" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 lg:gap-x-5 justify-center">
      <Category v-for="item in items" :key="item.id" :item="item"/>
    </div>
    <div v-else-if="items.length==0" class="flex flex-wrap items-center p-5 mx-2 bg-white border rounded">
      ...
      <div class="w-full pl-4 border-l md:w-1/2 flex flex-col gap-y-1">
