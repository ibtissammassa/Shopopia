<template>
  <div class="md:px-10 px-6 py-7 gap-y-7 flex flex-col justify-center">
    ...
    <div v-if="items.length >0" class="grid lg:grid-cols-3 gap-6 md:grid-cols-2 grid-cols-1">
    ...
    <div v-else-if="items.length==0" class="flex flex-wrap items-center p-5 mx-2 bg-white border rounded">
      ...
      <div class="w-full pl-4 border-l md:w-1/2 flex flex-col gap-y-1">
        ...
        <p>{{ 'Go to your store admin > blog > posts >' }} <a href="//business.storeino.com/stores/posts" target="_blank" class="text-primary font-bold ">{{ ' Add new' }}</a></p>
      </div>
    </div>
  </div>
</template>
