<template>
    <div>
            ...
            <div class="border-slate-200 border-b flex items-center px-5 md:px-10 py-2 justify-between">
                ...
                <div v-if="showWishlist || showCart" class="md:gap-x-6 gap-x-3 flex items-center">
                    <div v-if="showWishlist">
                        <nuxt-link to="/wishlist" class="flex items-center justify-center gap-x-1 hover:border-b-2 hover:border-blue-500 py-2" :class="$store.state.language.code=='AR' ? 'flex-row-reverse' : ''">
                            <div class="relative">
                            ...
                            <div class="hidden md:block">{{ $settings.header.wishlist.text }}</div>
                        </nuxt-link>
                    </div>
                    <div v-if="showCart">
                        <nuxt-link to="/cart" :class="$store.state.language.code=='AR' ? 'flex-row-reverse' : ''" class="flex items-center gap-x-1 hover:border-b-2 hover:border-blue-500 py-2">
                            <div class="relative">
                            ...
                            <div class="hidden md:block">{{ $settings.header.cart.text }}</div>
                        </nuxt-link>
                    </div>
        ...
        <AppLoader placement="AFTER_HEADER"/>
    </div>
...
export default {
    ...
    data(){
        return{
            ...
            categories:false,
            menutoggle: false,
            categoriesTitle: this.$settings.header.categories.buttonText,
            backgroundColor: this.$settings.header.color.background,
            textColor: this.$settings.header.color.text,
        }
    },
    methods:{
        toggleCategories(){
